...
db = Database()
utils = Utils(bot, db)
admins = list(map(lambda x: int(x), env["ADMIN_USERS"].split(",")))
...
async def promo_handler(message: types.Message) -> None:
...
async def genpromo_handler(message: types.Message) -> None:
    ...
    if " " not in message.text or len(message.text.split(" ")) != 3:
        return await message.answer("Usage: /genpromo $days[min: 1, max: 99999] $count[min: 1, max: 25]\n\nExample: /genpromo 7 25 ( 25 promo for 7 days )")
    ...
    try:
        ...
        if 0 >= days or days > 99999:
            return await message.answer("Invalid value in argument $days must be 1-99999")
        if 0 >= count or count > 25:
            return await message.answer("Invalid value in argument $count must be 1-25")
    except:
        return await message.answer("Invalid type in argument $days or $count must be numbers")
